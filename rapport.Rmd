...
- `wage_per_hour` : salaire mensuel (et non mensuel)
...
* *hispanique est ici utilisé comme un abus de language pour parler des populations originaire d'Amérique Latine*
...
## Migrations vers les US et entre les Etats. 
...
```{r}
insertion autre graph
```

Deuxièmement, l'éducation est très souvent genrée. De ce fait, les femmes sont souvent orientées vers des filières qui leur "correspondent" (stéréotype de genres) comme par exemple les métiers du "care" - car les femmes sont vues comme naturellement douées pour s'occuper des autres. Or ces filières genrées avec des métiers identifiés comme féminins sont les filières peu rémunératrices. 
...
ggplot(education, aes(y = major_industry_recode, x = wage_per_hour)) +
  ...
  labs(
    title = "Salaire mensuel moyen par branche",
    x = "Salaire mensuel moyen", y = "Branche"
  )  
```

Nous pouvons constater ici que les femmes sont majoritairement présentes dans les domaines tels que les services sociaux, l’hôpital et plus généralement le domaine médical (souvent dans des postes d’infirmières), ainsi que dans le domaine éducatif. Le domaine des services sociaux est peu rémunérateur, mais ceux de la médecine (en hôpital ou en dehors des hôpitaux) sont dans les plus rémunérateur. Cependant ce secteur représente une grande variété de métiers. 
Cependant, il convient tout de même de noter que les femmes sont aussi majoritaires présentes dans le domaine regroupant finance, assurance et immobilier qui est généralement très rémunérateur. 

```{r industrie-wage-boxplot}
ggplot(education, aes(y = major_industry_recode, x = wage_per_hour)) +
    geom_boxplot() +
    labs(
...
```{r gender-sector-precisely, fig.width=10}
...
Ce coût d’opportunité peut aussi expliquer une auto-censure des femmes dans leurs études. En effet, pourquoi investir (avec du temps et surtout de l’argent) alors qu’on gagnera moins que son mari ? 
...
La plus grande disparité entre hommes et femmes se fait au niveau PhD. 
...
Pour ce qui est de la corrélation entre diplôme et salaire par heure, on constate que le salaire croit en fonction du niveau du diplôme. Or, comme nous l’avons dit précédemment, les femmes sont moins enclines à continuer leurs études du fait d’un coût d’opportunité plus élevé que leurs homologues masculins. C’est en quelques sorte le serpent qui se mord la queue.  
```{r wage_education}
...
Nous allons ensuite nous intéresser à l'impact de l'ethnie sur le niveau de revenu. 
...
Nous allons maintenant nous intéresser aux différences entre les ethnies. Tout d'abord nous constatons que le salaire mensuel moyen maximum des personnes Afro-Américaines est plus faible que celui des autres ethnies. Cela peut s'expliquer par un racisme encore bien ancré dans la société américaine des années 90 qui se traduit par une discrimination à l'emploi. De plus, nous pouvons supposer que les personnes Afro-américaines sont plus représentées dans des secteurs peu rémunérateurs. 
...
ggplot(industry, aes(y = major_industry_recode, fill = race)) +
  geom_bar() +
  labs(x = "Nombre", y = "Type de branche")
```
...
Nous pouvons constater que les branches où travaillent majoritairement les personnes racisées (retail trade, public administration, manufacturing non-durable and durable goods, education) ne sont pas les branches les plus rémunératrices.
...
Nous pouvons remarquer ici, sur les branches qui nous intéressent (retail trade, public administration, manufacturing non-durable and durable goods, education) les salaires médians sont loin d’être les plus conséquents. 
...
Cet aspect politique a pu entrainer des migrations des personnes racisées, qui, nous l'avons dit tout à l'heure, ont un salaire moins élevés que leurs homologues blancs. 
La deuxième explication vient du fait que les Etats de la Sunbelt, dans les années 90, avait pour activité économique principale l'agriculture qui génère de moindre revenus. A cela s'ajoutait le tourisme et la pêche. L'industrie automobile venait de faire son apparition dans ces Etats, tout comme le domaine des hautes technologies qui n'était qu'à ses balbutiements. 
...
Les résultats sont sans équivoque ; ces facteurs ont une importance sur l'éducation, le choix de domaine d'activité, et le salaire.
Il est néanmoins intéressant de voir que cetaines minorités ethniques ont malgré tout réussi à optenir de hauts salaires moyens, et à accumuler du capital économique, voire scolaire. Au delà de l'ethnie, il pourrait donc y avoir un vrai enjeu à soutenir les populations les plus défavorisés en leur permettant l'accès à l'éducation, qui elle-même peut aider à obtenir de meilleurs salaires.
Malheureusement, presque 30 ans plus tard, les choses ne semblent pas avoir énormément évoluées. Les populations noires sont toujours les plus touchés par la pauvreté, et ont un revenu médian bien inférieur à d'autres groupes ethniques ( caucasiens, asiatiques).
La situation prends même un tournant innatendu avec l'apparition des nouvelles technologies, comme l'intelligence artificielle. Ce jeu de données par exemple, à été utilisé à des fins de machine learning. En analysant ces données, l'IA pourraient donc considérer que ces différences ethniques sont une norme sociale qu'il convient d'appliquer.
 Il y a donc un vrai risque qu'elle reproduise et accentue ces discriminations.
 Nous devons donc impérativement prendre conscience de ces nouveaux enjeux, afin de pas accentuer un cercle vicieux qui viserait à perpétuer ces discriminations.
