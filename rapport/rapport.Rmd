...
educationIndus <- data %>%
  ...
  group_by(educationIndus, race, sex, major_industry_recode) %>%
  summarise(wage_per_hour = mean(wage_per_hour))

ggplot(education, aes(y = major_industry_recode, x = wage_per_hour)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Wage per hour by industry",
    x = "Wage per hour", y = "Industry"
  )    )
```
```{r industrie-wage-boxplot, echo=FALSE}
ggplot(educationIndus, aes(y = major_industry_recode, x = wage_per_hour)) +
    geom_boxplot() +
    labs(
        title = "Wage per hour by industry",
        x = "Wage per hour", y = "Industrie"
    )
```
Nous pouvons constater ici que les femmes sont majoritairement présentes dans les domaines tels que les services sociaux, l’hôpital et plus généralement le domaine médical (souvent dans des postes d’infirmières), ainsi que dans le domaine éducatif. 
...
```
Ce coût d’opportunité peut aussi expliquer une auto-censure des femmes dans leurs études. En effet, pourquoi investir (avec du temps et surtout de l’argent) alors qu’on gagnera moins que son mari ? C’est le serpent qui se mord la queue, tout simplement parce que sur les premiers niveaux d’études, le niveau de diplôme à un impact sur le salaire. 
On constate que la part de femme en fonction du niveau d’étude va en diminuant, et qu’une large majorité des personnes qui ont arrêté leurs études au 1st grade (école élémentaire) sont… des femmes. 
Nous pouvons aussi noter qu’à notre grand étonnement, la majorité des personnes ayant un master permettant d’enseigner sont des hommes. En effet nous avions constaté précédemment que ce secteur était majoritairement féminin. 
...
```
En effet, pour ce qui est de la corrélation entre diplôme et salaire par heure, on constate que le salaire croit jusqu’au «high-school diploma». Sur ce segment d’étude (« high-school diploma »), les femmes semblent à parité avec les hommes ce qui n’entraine pas d’inégalités de ce côté-là. Cependant on constate que les titulaires d’un doctorat ou d’un master de professorat ont des salaires plus faibles que les personnes d’un diplôme légèrement inférieur. Or, nous avons vu que les personnes travaillant dans le domaine de l’enseignement sont majoritairement des femmes. 
...
Au niveau de l’éducation, selon notre jeu de données, la principale différence qui entraîne des inégalités entre les genres est que la majorité des personnes ayant arrêtés leurs études au 1st grade sont des femmes. 
