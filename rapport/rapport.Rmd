...
On constate que l'écrasante majorité de la population interrogée est blanche. 


A titre de comparaison, la population américaine en 1995 était répartie de la manière suivante (source : Census) : 
-+70% de la population était blanche
-12% de la population était noire
-10% de la population était hispanique
-3% de la population était asiatique

L’échantillon semble donc être globalement de la population américaine en 1994, avec tout de même une proportion de personnes noires légèrement moindre. 
...
ggplot(data = average_wph_by_age_sex, aes(
  ...
  labs(
    title = "Salaire horaire moyen en fonction de l'âge et du genre",
    x = "Age", y = "Salaire horaire moyen",
    color = "Genre"
  )
```
...
Deuxièmement, l'éducation est très souvent genrée. De ce fait, les femmes sont souvent orientées vers des filières qui leur "correspondent" (stéréotype de genres) comme par exemple les métiers du "care" - car bien évidement, les femmes sont naturellement douées pour s'occuper des autres. Or ces filières genrées avec des métiers identifiés comme féminins sont les filières peu rémunératrices. 

```{r industry-genre, echo=FALSE}
industry <- data %>%
...
ggplot(education, aes(y = major_industry_recode, x = wage_per_hour)) +
  ...
  labs(
    title = "Salaire horaire par branche",
    x = "Salaire horaire moyen", y = "Branche"
  )  
```
...
```{r industrie-wage-boxplot, echo=FALSE}
ggplot(education, aes(y = major_industry_recode, x = wage_per_hour)) +
    geom_boxplot() +
    labs(
        title = "Variance du salaire horaire en fonction du secteur d’activité",
        x = "Variance du salaire horaire", y = "Secteur d’activité"
    )
```
...
```{r gender-sector-precisely, echo=FALSE}
...
On constate que la part de femme en fonction du niveau d’étude va en diminuant, et qu’une large majorité des personnes qui ont arrêté leurs études au 1st grade (école élémentaire) sont… des femmes. La plus grande disparité entre hommes et femmes se fait au niveau PhD. 

```{r education-sex, echo=FALSE}
education <- data %>%
    filter(wage_per_hour != 0) %>%
    filter(education != "Children") %>%
    mutate(education = factor(
...
```{r levelracepercentage, echo=FALSE}
income_percent <- data %>%
...
Nous allons maintenant nous intéresser aux différences entre les ethnies. Tout d'abord nous constatons que le salaire horaire moyen maximum des personnes Afro-Américaines est plus faible que celui des autres ethnies. Cela peut s'expliquer par un racisme encore bien ancré dans la société américaine des années 90 qui se traduit par une discrimination à l'emploi. De plus, les personnes Afro-américaines nous pouvons supposer plus représentées dans des secteurs peu rémunérateurs. 

```{r industry_race, echo=FALSE}
industry <- data %>%
  filter(major_industry_recode != "Not in universe or children")
...
```
On constate encore une fois que le nombre de personnes blanches du jeu de données nuit à la lisibilité de nos graphiques axés sur les personnes racisées. Nous allons donc sélectionner seulement les personnes racisées. 

```{r industry_race2, echo=FALSE}
industry2 <- data %>%
...
ggplot(education, aes(y = major_industry_recode, x = wage_per_hour)) +
  geom_bar(stat = "summary") +
  labs(
    title = "Salaire horaire moyen par industrie",
    x = "Salaire horaire moyen", y = "Type de branche")  
```

Nous pouvons constater que les branches où travaillent majoritairement les personnes racisées (retail trade, public administration, manufacturing non-durable and durable goods, education) ne sont pas les branches les plus rémunératrices. Nous allons alors nous intéresser à la variance des salaires dans ces branches. 

```{r industry_wage3, echo=FALSE}
ggplot(education, aes(y = major_industry_recode, x = wage_per_hour)) +
  geom_boxplot() +
  labs(
    title = "Salaire horaire par branche",
    x = "Salaire horaire", y = "Branche"
  )
```
...
```{r occupation_race_W, echo=FALSE}
occupation <- data %>%
...
On constate que les personnes racisées sont très présentes dans certains secteurs, et dans des proportions toujours similaires en fonction des ethnies, blancs compris. (à voir si on laisse ou pas parce que c’est pas très intéressant du coup). 

Nous allons pouvoir nous intéresser à l’aspect scolaire des inégalités entre les ethnies. 

```{r education_race, echo=FALSE}
ggplot(education, aes(y = education, fill = race)) +
    geom_bar(position = "fill")
```

On constate ici que jusqu’au High school diploma les personnes racisées sont autant présentes que les personnes blanches, voir même sont majoritaires dans certains cas. Cependant cette tendance s’inverse pour les diplômes les plus valorisés (et les plus longs à obtenir) que sont les prof school degree, master degree ou bien encore phD. Or nous avons vu tout à l’heure que ces diplômes étaient les plus rémunérateurs. 
